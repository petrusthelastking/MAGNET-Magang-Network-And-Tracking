...
test.describe('Admin - Data Dosen CRUD Operations', () => {
  ...
  test('should open Tambah Dosen modal and display all required fields', async ({ page }) => {
    console.log('✓ Testing Tambah Dosen modal...');
    ...
    // Find and click "Tambah Dosen" button with longer timeout
    const tambahButton = page.locator('button').filter({ hasText: /Tambah Dosen/i });
    await expect(tambahButton).toBeVisible({ timeout: 15000 });
    console.log('✓ Found Tambah Dosen button');
    ...
    const modalTitle = page.locator('text=/Tambahkan data dosen baru/i');
  ...
  test('should show validation errors when submitting empty Tambah Dosen form', async ({ page }) => {
  ...
  test('should be able to edit dosen data', async ({ page }) => {
    console.log('✓ Testing edit dosen functionality...');

    await navigateToDataDosen(page);

    // Find and click edit button on first row
    const editButton = page.locator('tr')
      .first()
      .locator('button, a')
    ...
    if (hasEditButton) {
      ...
      // Verify edit form/modal appears
      const editForm = page.locator('form').filter({
        hasText: /Edit|Ubah Data Dosen/i
      }).first();

      const formVisible = await editForm.isVisible().catch(() => false);

      if (formVisible) {
        console.log('✓ Edit form/modal is visible');

        // Verify form fields are populated
        const namaInput = page.locator('input[wire\\:model*="editDosenNama"]').or(
          page.locator('label:has-text("Nama")').locator('..').locator('input')
        );
        const hasValue = await namaInput.inputValue() !== '';

        if (hasValue) {
          console.log('✓ Form fields are pre-populated with existing data');
        }

        // Try to submit the form without changes
  ...
  test('should be able to delete dosen data', async ({ page }) => {
    console.log('✓ Testing delete dosen functionality...');

    await navigateToDataDosen(page);

    // Find and click delete button on first row
    const deleteButton = page.locator('tr')
      .first()
      .locator('button, a')
    ...
    if (hasDeleteButton) {
      ...
      const confirmDialog = page.locator('div[role="dialog"]').or(
        page.locator('.modal').or(
          page.locator('div').filter({ hasText: /Konfirmasi|Hapus|Delete/i })
        )
      );
      ...
      if (hasConfirmDialog) {
        ...
        const confirmButton = page.locator('button').filter({
          hasText: /Ya|Hapus|Delete|Confirm/i
        }).first();
