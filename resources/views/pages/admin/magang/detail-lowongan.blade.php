...
mount(function (int $id) {
    try {
        $this->lowongan_magang = Magang::findOrFail($id);
    ...
    @if ($isDataFound)
        <div class="min-h-full flex gap-8 items-start">
            <div class="flex flex-col items-center bg-white p-4 rounded-xl shadow-md w-96 max-w-96">
                <img src="{{ asset('img/company-cisco.png') }}" alt="Logo Perusahaan"
                ...
                <div class="card-body gap-4">
                    ...
                    <div class="space-y-4">
                        ...
                        <flux:field>
                            ...
                            @if ($isEditing)
                                <flux:select placeholder="Perusahaan" wire:model="perusahaan_id">
                                    @foreach ($perusahaan_list as $perusahaan)
                                        <flux:select.option value="{{ $perusahaan }}">{{ $perusahaan }}
                                        </flux:select.option>
                                    @endforeach
                                </flux:select>
                            @else
                            ...
                            @endif
                            <flux:error name="perusahaan" />
                        </flux:field>

                        <flux:field>
                            <flux:label>Lokasi</flux:label>
                            @if ($isEditing)
                                <flux:input value=""" wire:model="lokasi" />
                            @else
                                <flux:input value="" wire:model="lokasi" readonly
                                    class="caret-transparent" />
                            @endif
                        ...
                        <flux:field>
                            ...
                            @if ($isEditing)
                                <flux:input wire:model="maks_kuota" />
                            @else
                                <flux:input wire:model="maks_kuota" readonly
                                    class="caret-transparent" />
                            @endif
                            <flux:error name="maks_kuota" />
                        </flux:field>

                        <flux:field>
                            <flux:label>Jumlah Pendaftar</flux:label>
                                <flux:input wire:model="jumlah_pendaftar" readonly
                                    class="caret-transparent" />
                            <flux:error name="jumlah_pendaftar" />
                        </flux:field>

                        <flux:field>
                            <flux:label>Sisa Kuota</flux:label>
                                <flux:input wire:model="sisa_kuota" readonly
                                    class="caret-transparent" />
                            <flux:error name="sisa_kuota" />
                        </flux:field>
                    </div>
                </div>
            </div>
