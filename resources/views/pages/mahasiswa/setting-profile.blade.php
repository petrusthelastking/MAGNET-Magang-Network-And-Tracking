    ...
    <div class="gap-3 flex flex-col">
        <div class="card bg-white shadow-md">
            <div class="card-body p-5">
                ...
                <div class="grid grid-cols-2 gap-3 mt-4">
                    ...
                    <flux:input readonly value="{{ $mahasiswa->programStudi->nama ?? '-' }}" type="text" label="Program Studi" />
        ...
        <div class="card bg-white shadow-md">
            <div class="card-body grid grid-cols-2 gap-3">
                <flux:input readonly value="{{ $preferensi->keahlian ?? '-' }}" type="text" label="Skill yang anda miliki" />
                <flux:input readonly value="{{ $preferensi->bidang_inustri ?? '-' }}" type="text" label="Bidang industri" />
                <flux:input readonly value="{{ $preferensi->pekerjaan_impian ?? '-' }}" type="text" label="Pekerjaan impian" />
                <flux:input readonly value="{{ number_format($preferensi->upah_minimum ?? 0, 0, ',', '.') }}" type="text" label="Upah minimum" />
                <flux:input readonly value="{{ $preferensi->lokasi_magang ?? '-' }}" type="text" label="Lokasi magang" />
            </div>

            <div class="card-actions flex justify-end p-5">
                <a href="{{ route('mahasiswa.preference.edit') }}">
